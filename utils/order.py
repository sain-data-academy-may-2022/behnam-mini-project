...
# ---------------------------------------
def add_order(products_list, couriers_list, insert_order_into_db=insert_order_into_db, read_orders=read_orders):
...
def update_order_status(orders_list, update_order_status_on_db=update_order_status_on_db, read_orders=read_orders):
    try:
...
def update_order_courier(orders_list, couriers_list, update_order_courier_on_db=update_order_courier_on_db, read_orders=read_orders):
    try:
        order_index = int(input('enter order id to update: '))
        assert orders_list[order_index], 'wrong order_id'
        order_id = orders_list[order_index][0]
...
def delete_order(orders_list, delete_order_from_db=delete_order_from_db, read_orders=read_orders):
    try:
